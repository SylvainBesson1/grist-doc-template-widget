...
function processLoops(html, forPdf) {
  ...
  // Traitement des boucles imbriquées pour deux colonnes de référence multiple
  var multiRefLoopRegex = /<!--LOOP:MULTIREF:([^:]+):([^-]+)-->([\s\S]*?)<!--\/LOOP-->/gi;
  resolved = resolved.replace(multiRefLoopRegex, function(match, firstRefCol, secondRefCol, loopContent) {
    return executeLoopForMultiRef(firstRefCol, secondRefCol, loopContent, forPdf);
  });
